...
const StoreContextProvider = (props) => {
    ...
    const addToCart = async (itemId) => {
        ...
        if (token) {
            await axios.post('https://food-delivery-api-tau.vercel.app/api/cart/add', { itemId }, {
                headers: { token }
            });
        }
    };
    ...
    const loadCartData = async (token) => {
        try {
            const response = await axios.post('https://food-delivery-api-tau.vercel.app/api/cart/get', {itemId}, {
                headers: { token }
            });
